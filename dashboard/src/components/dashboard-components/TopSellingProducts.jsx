// dashboard/src/components/dashboard-components/TopSellingProduct.jsx
...
const TopSellingProduct = () => {
    ...
    return (
        <div className="bg-white rounded-xl p-6 shadow-md overflow-x-auto xl:col-span-2 max-h-[440px] scrollbar-hide">
            ...
            <p className="text-sm text-gray-500 mb-4">Products having most sales in Nepa</p>
                    ...
                    {topSellingProducts?.length > 0 && topSellingProducts.map((element, index) => (
                        <tr key={index} className="border-b hover:bg-gray-50 transition">
                            <td className="px-4 py-3">
                                <img src={element.images?.[0]?.url || element.image} alt={element.name} className="w-12 h-12 object-cover rounded-md" />
                            </td>
                            ...
                            <td className="px-4 py-3 text-yellow-500 font-semibold">{element.ratings || "0.00"}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
